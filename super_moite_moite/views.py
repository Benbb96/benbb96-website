...
def liste_logements(request):
    form = LogementForm(request.POST or None, profil=request.user.profil)
    if form.is_valid():
        logement = form.save()
    ...
    return render(
        request,
        "super_moite_moite/logement_list.html",
        {"logements": logements, "form": form},
    )
...
def dupliquer_logement(request, slug):
    ...
    # Copie du logement
    duplicata_logement = Logement.objects.create(nom="Copie de " + logement.nom)
    # Ajout des habitants
    ...
    for categorie in logement.categories.all():
        duplicata_categorie = duplicata_logement.categories.create(
            nom=categorie.nom, order=categorie.order, couleur=categorie.couleur
        )
        ...
        for tache in categorie.taches.all():
            ...
            for point_defaut in tache.point_profils.all():
                duplicata_tache.point_profils.create(
                    profil=point_defaut.profil, point=point_defaut.point
                )

    messages.success(request, f"Le logement {logement} a bien été dupliqué.")
