...
from music.models import Artiste, Label, Musique, Playlist, Style
...
class MusiqueFilter(django_filters.FilterSet):
    has_link = django_filters.BooleanFilter(
        label=_("Has links"), method="search_has_link"
    )
...
class StyleFilter(django_filters.FilterSet):
    ...
    nom = django_filters.CharFilter(
        ...
        widget=forms.TextInput(
            attrs={"placeholder": _("Search"), "aria-label": _("Name")}
        ),
    )
...
class ArtisteFilter(django_filters.FilterSet):
    text = django_filters.CharFilter(label=_("Text"), method="search_text")
    styles = _styles_filter()
