...
class Playlist(models.Model):
    ...
    createur = models.ForeignKey(
        Profil, related_name="playlists_crees", on_delete=models.CASCADE
    )
...
class Artiste(models.Model):
    ...
    telephone_contact = models.CharField(
        "téléphone de contact", max_length=15, blank=True
    )
    ...
    def all_musics(self):
        return Musique.objects.filter(
            Q(artiste=self) | Q(remixed_by=self) | Q(featuring=self)
        )
    ...
    def soundcloud_followers(self):
        if self.soundcloud_id:
            try:
                ...
                print(
                    f"Erreur sur l'appel API du nombre de followers de l'artiste {self} :"
                )
                print(e)
            else:
...
class Musique(models.Model):
    ...
    label = models.ForeignKey(
        Label, on_delete=models.SET_NULL, related_name="musiques", null=True, blank=True
    )
    styles = models.ManyToManyField(Style, related_name="musiques", blank=True)
    playlists = models.ManyToManyField(
        Playlist, related_name="musiques", blank=True, through="MusiquePlaylist"
    )
    createur = models.ForeignKey(
        Profil,
        related_name="musiques_crees",
        on_delete=models.SET_NULL,
        null=True,
        blank=True,
    )
    ...
    def __str__(self):
        return f"{self.artiste_display()} - {self.titre_display()}"

    def artiste_display(self):
        featuring = self.featuring.all()
        if featuring:
            return " & ".join(
                [self.artiste.nom_artiste]
                + list(artiste.nom_artiste for artiste in featuring)
            )
    ...
    def titre_display(self):
        if self.remixed_by:
            return f"{self.titre} ({self.remixed_by.nom_artiste} Remix)"
        return self.titre
...
class MusiquePlaylist(SortableMixin):
    ...
    date_ajout = models.DateTimeField(
        "date d'ajout de la musique dans la playlist", auto_now_add=True
    )
...
class BaseLien(models.Model):
    ...
    plateforme = models.ForeignKey(
        Plateforme, on_delete=models.SET_NULL, null=True, blank=True
    )
    date_creation = models.DateTimeField("date de création", auto_now_add=True)
...
class LienPlaylist(BaseLien):
    playlist = models.ForeignKey(
        Playlist, on_delete=models.CASCADE, related_name="liens"
    )
...
class Lien(BaseLien):
    musique = models.ForeignKey(
        Musique, on_delete=models.CASCADE, verbose_name=_("Music"), related_name="liens"
    )
