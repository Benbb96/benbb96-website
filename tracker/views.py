...
from tracker.forms import SelectTrackersForm, TrackerForm, TrackForm
from tracker.models import Track, Tracker
from tracker.serializers import (
    CustomTrackSerializer,
    TrackerLightSerializer,
    TrackerSerializer,
    TrackSerializer,
)
...
class TrackerView(ModelViewSet):
    ...
    def reorder(self, request):
        ...
        if set(ids) != {t.id for t in trackers}:
            return Response(
                {"error": "Liste d'IDs incomplète ou invalide"},
...
def tracker_list(request):
    trackers = (
        Tracker.objects.filter(createur=request.user.profil)
        .annotate(
            tracks_count=Count("tracks"),
            last_track_date=Max("tracks__datetime"),
        )
        .order_by("order")
    )  # l'annotation (GROUP BY) écrase l'ordre par défaut du Meta → on le réimpose
    ...
    return render(
        request, "tracker/tracker_list.html", {"trackers": trackers, "form": form}
    )
...
def tracker_quick_add(request, pk):
    tracker = get_object_or_404(
        Tracker.objects.filter(createur=request.user.profil), id=pk
    )
    if tracker.type == Tracker.TYPE_MESURE:
        messages.info(
            request,
            "Ce tracker nécessite une valeur, veuillez l'ajouter depuis la page de détail.",
        )
        return redirect(tracker)
    Track.objects.create(tracker=tracker)
    messages.success(request, f"Un track a bien été ajouté sur le tracker {tracker}")
    return redirect("tracker:liste-tracker")
...
def tracker_detail(request, pk):
    ...
    return render(
        request,
        "tracker/tracker_detail.html",
        {"tracker": tracker, "tracks": tracks, "form": form},
    )
...
def get_tracks_from_request(request):
    ...
    for tracker_id in ids:
        tracker = get_object_or_404(
            Tracker.objects.filter(createur=request.user.profil), id=tracker_id
        )
...
def tracker_data(request):
    ...
    for tracker, tracks in trackers.items():
        ...
        if tracker.type == Tracker.TYPE_MESURE:
            ...
            df.loc[:, "datetime"] = pd.to_datetime(df["datetime"]).dt.tz_convert(
                "Europe/Paris"
            )
            ...
            for i, label in enumerate(data_mean.index.tolist()):
                ...
                val = {
                    "mean": None
                    if pd.isna(data_mean.iloc[i])
                    else round(float(data_mean.iloc[i]), 2),
                    "min": None
                    if pd.isna(data_min.iloc[i])
                    else float(data_min.iloc[i]),
                    "max": None
                    if pd.isna(data_max.iloc[i])
                    else float(data_max.iloc[i]),
                }
            ...
            delta = (
                tracks.latest("datetime").datetime.date()
                - tracks.earliest("datetime").datetime.date()
            )
            ...
            averages.append(
                {"tracker": tracker.nom, "avg": round(avg, 2), "isValeur": False}
            )
    ...
    for dataset in datasets:
        if dataset["trackerType"] == Tracker.TYPE_MESURE:
            dataset["data"] = [
                labels[label].get(dataset["label"], {}).get("mean")
                for label in sorted_labels
            ]
            dataset["minData"] = [
                labels[label].get(dataset["label"], {}).get("min")
                for label in sorted_labels
            ]
            dataset["maxData"] = [
                labels[label].get(dataset["label"], {}).get("max")
                for label in sorted_labels
            ]
        else:
            dataset["data"] = [
                labels[label].get(dataset["label"], {}).get("count", 0)
                for label in sorted_labels
            ]
...
def format_timedelta(td):
    ...
    if days > 0:
        return f"{days} jour{'s' if days > 1 else ''} {hours}h {minutes}m {seconds}s"
    elif hours > 0:
    ...
    if deltas and min_1 and min_2 and max_1 and max_2:
        ...
        delta_stats = {
            "deltaMin": format_html(
                ...
                format_timedelta(minimum),
                min_1.strftime(date_format),
                min_2.strftime(date_format),
            ),
            "deltaAvg": format_timedelta(sum(deltas, timedelta(0)) / len(deltas)),
            "deltaMax": format_html(
                "<b>{}</b> <br><small>Entre le {} et le {}</small>",
                format_timedelta(maximum),
                max_1.strftime(date_format),
                max_2.strftime(date_format),
            ),
        }
    ...
    return JsonResponse(
        {
            "html": render_to_string(
                ...
                {
                    "tracks": sorted(
                        all_tracks, key=lambda item: item.datetime, reverse=True
                    ),
                    "compare": True,
                    "show_valeur": show_valeur,
                },
                request,
            ),
