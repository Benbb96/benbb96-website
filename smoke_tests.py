...
class PublicViewsSmokeTest(TestCase):
    ...
    def _get200(self, view_name, *args, **kwargs):
        ...
        self.assertEqual(r.status_code, 200, f"Vue {view_name!r} → attendu 200, obtenu {r.status_code}")
...
class AuthRequiredViewsSmokeTest(TestCase):
    ...
    def _expect_redirect(self, view_name):
        ...
        self.assertEqual(r.status_code, 302, f"Vue {view_name!r} → attendu 302, obtenu {r.status_code}")
...
class TrackerAPITest(TestCase):
    ...
    def _get200(self, view_name, query=''):
        ...
        self.assertEqual(r.status_code, status.HTTP_200_OK, f"API {view_name!r}{query} → {r.status_code}")
        return r
...
class SMMAPITest(TestCase):
    ...
    def _get200(self, view_name):
        ...
        self.assertEqual(r.status_code, status.HTTP_200_OK, f"API {view_name!r} → {r.status_code}")
        return r
