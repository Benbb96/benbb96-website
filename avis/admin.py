...
from .models import Avis, CategorieProduit, Produit, Structure, TypeStructure
...
class StructureAdmin(admin.ModelAdmin):
    ...
    def position_map(self, instance):
        if instance.adresse is not None:
            ...
            return format_html(
                f'<img src="http://maps.googleapis.com/maps/api/staticmap?key={settings.GOOGLE_API_KEY[0]}&center={instance.adresse.latitude},{instance.adresse.longitude}&zoom={14}&size={100}x{100}&maptype=roadmap&markers={instance.adresse.latitude},{instance.adresse.longitude}&sensor=false&visual_refresh=true&scale={2}" width="{100}" height="{100}">'
            )
...
class AvisAdmin(PhotoAdminAbtract):
    ...
    search_fields = (
        "produit__nom",
        "produit__structure__nom",
        "auteur__user__username",
    )
    date_hierarchy = "date_creation"
