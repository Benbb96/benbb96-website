...
class TypeStructure(models.Model):
    ...
    categories = models.ManyToManyField(
        CategorieProduit,
        help_text="Selectionnez la ou les catégories de produit possibles pour ce type de structure.",
    )
...
class Structure(models.Model):
    ...
    type = models.ForeignKey(
        TypeStructure, on_delete=models.PROTECT, verbose_name="type de la structure"
    )
    informations = models.TextField(
        blank=True, help_text="Informations utiles et relatives à la structure."
    )
    lien = models.URLField(
        max_length=255, blank=True, help_text="Le lien vers le site internet."
    )
    telephone = models.CharField(
        max_length=20,
        blank=True,
        help_text="Indicatif facultatif et sans espaces.",
...
class ProduitManager(models.Manager):
    def get_queryset(self):
        return (
            super()
            .get_queryset()
            .select_related("structure__type")
            .prefetch_related("avis_set")
            .annotate(
...
class Produit(models.Model):
    ...
    categories = models.ManyToManyField(
        CategorieProduit,
        help_text="Sélectionnez la ou les catégories de ce produit.",
        blank=True,
    )
...
class Avis(PhotoAbstract):
    ...
    avis = models.TextField(
        blank=True, help_text="Ton avis en quelques mots sur le produit."
    )
    ...
    def __str__(self):
        return f"{self.auteur} : {self.produit.nom} ({self.note}/10)"
    ...
    def has_been_edited(self):
        ...
        return self.date_creation.replace(microsecond=0) != self.date_edition.replace(
            microsecond=0
        )
