...
class SpotAdmin(admin.ModelAdmin):
    ...
    def position_map(self, instance):
        if instance.position is not None:
            ...
            return format_html(
                f'<img src="http://maps.googleapis.com/maps/api/staticmap?key={settings.GOOGLE_API_KEY[0]}&center={instance.position.latitude},{instance.position.longitude}&zoom={14}&size={100}x{100}&maptype=roadmap&markers={instance.position.latitude},{instance.position.longitude}&sensor=false&visual_refresh=true&scale={2}" width="{100}" height="{100}">'
            )
